...
Make sure to add compress: false in your next.config.ts(next 15) or next.config.mjs(next 14). [Reference](https://nextjs.org/docs/app/api-reference/next-config-js/compress) 
...
Add the following configuration to the file. Last two configuration is there to enable streaming.
...
```bash
sudo ln -s /etc/nginx/sites-available/nextjs-todo /etc/nginx/sites-enabled/
```
