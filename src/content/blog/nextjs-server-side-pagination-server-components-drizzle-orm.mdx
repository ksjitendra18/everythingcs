...
<CodeContainer fileName="terminal">
  ```bash mysql -u username -p # enter password ```
</CodeContainer>

<CodeContainer fileName="terminal">
  ```bash create database paginationguide; ```
</CodeContainer>

Now create a .env.local file and add the database url.

<CodeContainer fileName="terminal">
  ```bash DB_URL=mysql://username:password@localhost:3306/paginationguide ```
</CodeContainer>
