---
title: "Server Side Pagination in Next.js in Server Components using Drizzle ORM"
...
<img src="https://static.everythingcs.dev/blog/pagination-nextjs-291023/seed-db.png" loading="lazy"/>
...
export default async function Home({ searchParams }: SearchParamsProps) {
  ...
  const allUsers = await db.select().from(users).limit(numberOfItems).offset(offsetItems);
  return (
