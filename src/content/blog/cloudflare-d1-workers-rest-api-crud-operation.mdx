...
<CodeContainer fileName="terminal">```bash npm i ```</CodeContainer>

Then run the following command to run the react program

<CodeContainer fileName="terminal">```bash npm run dev ```</CodeContainer>
...
<CodeContainer fileName="api/src/db/schema.sql">
  ```sql DROP TABLE IF EXISTS todos; CREATE TABLE IF NOT EXISTS todos (id
  INTEGER PRIMARY KEY, todo TEXT NOT NULL, is_completed INTEGER NOT NULL DEFAULT
  0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP); ```
</CodeContainer>
...
<CodeContainer fileName="terminal">
  ```bash wrangler d1 execute todos-app --local --file=./src/db/schema.sql ```
</CodeContainer>
...
<CodeContainer>
  ```ts await c.env.DB.prepare("INSERT INTO todos (todo) values
  (?)").bind(todo).run(); ```
</CodeContainer>
...
<CodeContainer fileName="terminal">
  ```bash wrangler d1 execute todos-app --file=./src/db/schema.sql ```
</CodeContainer>
...
<CodeContainer fileName="terminal">```bash npm run deploy ```</CodeContainer>
