...
We will make use of Astro SSR, API routes and database session strategy to implement authentication and also persist it. Also we will make use of middlewares to protect the pages and API routes. We are going to use DrizzleORM to handle db schema and queries,
...
This post is part 1 in the series of exploring auth in astro framework. In second part we will implement the email password based authentication and in third part we will implement the passwordless magic email based authentication. In the final part we will implement the 2-Factor authentication.
...
- Implement PCKE which is currently (supported by google)
...
Download the starter code from the repo(oauth starter branch). [Direct download](https://github.com/ksjitendra18/astro-js-auth-oauth-passwordless-credentials/archive/refs/heads/oauth-starter.zip) or you can clone the oauth-starter branch.
...
npm i drizzle-orm @libsql/client @paralleldrive/cuid2
npm i -D drizzle-kit
```
...
if (!userExists) {
  ...
  return new Response(null, {
    ...
    headers: {
      Location: "/profile",
      "Set-Cookie": `app_auth_token=${
        newSession[0].id
      }; Path=/; HttpOnly; SameSite=Lax;Expires=${expiresAt.toUTCString()}; Secure=${
...
In the accounts page we will display the user email, their connected accounts and also their login logs.
