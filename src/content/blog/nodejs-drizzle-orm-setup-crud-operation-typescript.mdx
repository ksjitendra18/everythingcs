...
<CodeContainer fileName="terminal">
  ```bash create database drizzlenode; ```
</CodeContainer>

You can verified it is created or not by running command. You will see all the databases.

<CodeContainer fileName="terminal">```bash show databases; ```</CodeContainer>

Now create a .env file in root folder

<CodeContainer fileName=".env">
  ```bash DB_URL="mysql:username@password:3306/drizzlenode" ```
</CodeContainer>
...
<CodeContainer fileName="terminal">
  ```bash npm install drizzle-orm mysql2 npm install drizzle-kit -D ```
</CodeContainer>
...
<CodeContainer fileName="terminal">```js npm run db:generate ```</CodeContainer>
...
<CodeContainer fileName="terminal">```js npm run db:push ```</CodeContainer>
...
<CodeContainer>
  ```ts import {db} from "../../db/setup"; import {users} from
  "../../db/schema"; ... await db.select().from(users); ```
</CodeContainer>
...
<CodeContainer>
  ```ts import {db} from "../../db/setup"; import {users} from
  "../../db/schema"; ... await db.select().from(users).where(eq(users.id,
  Number(userId))); ```
</CodeContainer>
