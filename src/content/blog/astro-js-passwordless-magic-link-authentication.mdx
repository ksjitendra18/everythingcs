  ...
  try {
    ...
    if (userExists) {
      ...
      await createLoginLog({
        ...
        userId: userExists.id,
        ip: Astro.request.headers.get("x-real-ip") ?? "dev",
      });
...
So in this post I implemented the magic link based authentication. The completed code can be found in the [repo](https://github.com/ksjitendra18/astro-js-auth-oauth-passwordless-credentials) under the [magic-link-final](https://github.com/ksjitendra18/astro-js-auth-oauth-passwordless-credentials/tree/magic-link-starter) branch.

 This the 3rd part of the [Astro Authentication Series](https://everythingcs.dev/category/astro-authentication/). In the next part I will implement multi factor authentication. Join the [EverythingCS discord server](https://discord.gg/W3nubBNeXK) if you have any further query and for future post updates.
